...
function StatCard({ label, value, icon, color, bg }: any) {
  ...
  return (
    <Animated.View style={[s.statCard, { transform: [{ scale: anim }] }]}>
      <LinearGradient colors={[bg, '#fff']} style={s.statGrad} start={{x:0,y:0}} end={{x:1,y:1}}>
...
export default function AdminScreen() {
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: (_, vars) => {
      ...
      toast.success(`تم تغيير الحالة إلى: ${STATUS[vars.status]?.label}`);
    },
  });
    ...
    <SafeAreaView style={s.container}>
      ...
      <LinearGradient colors={[PRIMARY, '#0a5566']} style={s.header} start={{x:0,y:0}} end={{x:1,y:1}}>
        ...
        <View style={s.headerCenter}>
          ...
          <Text style={s.headerSub}>مرحباً بك مدير النظام 👋</Text>
        </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        style={s.statsRow} contentContainerStyle={{ padding: 16, gap: 12 }}>
        <StatCard label="إجمالي الإيرادات" value={`${(totalRevenue/1000).toFixed(0)}k د.ع`}
          icon="cash-outline" color="#10b981" bg="#ecfdf5" />
        <StatCard label="صافي الأرباح" value={`${(totalProfit/1000).toFixed(0)}k د.ع`}
          icon="trending-up-outline" color="#3b82f6" bg="#eff6ff" />
        <StatCard label="طلبات معلقة" value={pendingCount}
          icon="time-outline" color="#f59e0b" bg="#fffbeb" />
        <StatCard label="إجمالي الطلبات" value={orders.length}
          icon="bag-outline" color={PRIMARY} bg="#f0f9ff" />
        <StatCard label="التجار النشطين" value={merchantCount}
          icon="storefront-outline" color="#8b5cf6" bg="#f5f3ff" />
        <StatCard label="المنتجات" value={products.length}
          icon="cube-outline" color={SECONDARY} bg="#fffbeb" />
      </ScrollView>
      ...
      <View style={s.searchBox}>
        ...
        <TextInput style={s.searchInput}
          placeholder={tab==='orders' ? 'ابحث باسم الزبون أو الهاتف أو رقم الطلب...' : 'ابحث عن منتج...'}
      ...
      {tab === 'orders' && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false}
          style={s.filterRow} contentContainerStyle={{ gap: 8, paddingHorizontal: 16 }}>
          {[['all','الكل','grid-outline'],
            ['pending','انتظار','time-outline'],
            ['processing','معالجة','refresh-outline'],
            ['delivered','مُسلَّم','checkmark-circle-outline'],
            ['returned','مرتجع','arrow-undo-outline']
          ].map(([key, label, icon]) => (
            <TouchableOpacity key={key}
              style={[s.filterBtn, orderFilter===key && s.filterBtnActive]}
              onPress={() => setOrderFilter(key)}>
              <Ionicons name={icon as any} size={14}
                color={orderFilter===key ? '#fff' : '#6b7280'} />
      ...
      <ScrollView style={{ flex:1 }}
          ...
          <View key={order.id} style={s.orderCard}>
            <View style={s.orderHead}>
              <View style={[s.statusPill, { backgroundColor: STATUS[order.status]?.bg }]}>
                ...
                <Text style={[s.statusPillText, { color: STATUS[order.status]?.color }]}>
            ...
            <View style={s.orderBody}>
              <View style={s.orderRow}>
                <Text style={s.orderVal}>{order.customerName}</Text>
                <Text style={s.orderLbl}>👤 الزبون</Text>
              </View>
              <View style={s.orderRow}>
                <Text style={s.orderVal}>{order.customerPhone}</Text>
                <Text style={s.orderLbl}>📞 الهاتف</Text>
              </View>
              <View style={s.orderRow}>
                <Text style={s.orderVal}>{order.province}</Text>
                <Text style={s.orderLbl}>📍 المحافظة</Text>
              </View>
              <View style={s.orderRow}>
                <Text style={[s.orderVal, {color: SUCCESS, fontWeight:'bold'}]}>
                  {order.totalProfit?.toLocaleString()} د.ع
                </Text>
                <Text style={s.orderLbl}>💰 الربح</Text>
              </View>
              <View style={s.orderRow}>
                <Text style={[s.orderVal, {color: PRIMARY, fontWeight:'bold', fontSize:16}]}>
        ...
        {tab === 'products' && (
          <>
            <TouchableOpacity style={s.addBtn} onPress={() => setShowAddProduct(true)}>
              <LinearGradient colors={[PRIMARY, '#0a8a9f']} style={s.addBtnGrad}
            ...
            {filteredProducts.map((p: any) => (
              <View key={p.id} style={s.productCard}>
                ...
                <View style={s.productInfo}>
                  <View style={s.productCatPill}>
                    <Text style={s.productCatText}>{p.category}</Text>
                  </View>
                  <Text style={s.productName}>{p.name}</Text>
                  <Text style={s.productDesc} numberOfLines={1}>{p.description}</Text>
                  <View style={s.productPrices}>
                    <View>
                      <Text style={s.priceLabel}>أدنى بيع</Text>
                      <Text style={[s.priceVal, {color: SECONDARY}]}>{p.sellingPriceMin?.toLocaleString()} د.ع</Text>
                    </View>
                    <View>
                      <Text style={s.priceLabel}>الجملة</Text>
                      <Text style={[s.priceVal, {color: '#6b7280'}]}>{p.wholesalePrice?.toLocaleString()} د.ع</Text>
                    </View>
                    <View>
                      <Text style={s.priceLabel}>الربح</Text>
                      <Text style={[s.priceVal, {color: SUCCESS}]}>
                        {(p.sellingPriceMin - p.wholesalePrice)?.toLocaleString()} د.ع
                      </Text>
                    </View>
                  </View>
                  <View style={s.stockRow}>
                    <View style={[s.stockPill, p.stock < 5 && {backgroundColor:'#fef2f2'}]}>
                      ...
                      <Text style={[s.stockText, p.stock < 5 && {color: DANGER}]}>
                        {p.stock < 5 ? '⚠️ ' : ''}مخزون: {p.stock}
                      </Text>
                    </View>
                  </View>
                </View>
                <View style={s.productActions}>
                  ...
                  <TouchableOpacity style={s.deleteBtn} onPress={() => deleteProduct.mutate(p.id)}>
        ...
        {tab === 'users' && users.filter((u:any)=>u.role!=='admin').map((u: any) => (
          <View key={u.id} style={s.userCard}>
            <LinearGradient colors={[PRIMARY+'15', '#fff']}
              style={s.userGrad} start={{x:0,y:0}} end={{x:1,y:1}}>
              ...
              <View style={s.userStats}>
                <View style={s.userStatBox}>
                  <Text style={[s.userStatVal, {color: SUCCESS}]}>{u.balance?.toLocaleString()}</Text>
                  <Text style={s.userStatLabel}>الرصيد المتاح</Text>
                </View>
                <View style={[s.userStatBox, {borderRightWidth:1, borderRightColor:'#e5e7eb'}]}>
                  <Text style={[s.userStatVal, {color: SECONDARY}]}>{u.pendingBalance?.toLocaleString()}</Text>
            ...
            <View style={s.modalCard}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                {[
                  ...
                  { label:'الوصف', key:'description', placeholder:'وصف مختصر للمنتج', keyboard:'default' },
                  { label:'سعر الجملة (د.ع) *', key:'wholesalePrice', placeholder:'0', keyboard:'numeric' },
                  { label:'أدنى سعر بيع (د.ع) *', key:'sellingPriceMin', placeholder:'0', keyboard:'numeric' },
                ...
                {form.imageUrl ? (
                  <Image source={{ uri: form.imageUrl }}
                    style={s.previewImg} resizeMode="cover" />
                ) : null}
              ...
              <TouchableOpacity style={s.confirmBtn}
                ...
                <LinearGradient colors={[PRIMARY, '#0a8a9f']} style={s.confirmGrad}
                  start={{x:0,y:0}} end={{x:1,y:0}}>
...
const s = StyleSheet.create({
  ...
  statsRow: { maxHeight: 120 },

  statCard: { backgroundColor:'#fff', borderRadius:16, padding:14, minWidth:130,
    shadowColor:'#000', shadowOpacity:0.06, shadowRadius:10, elevation:3 },
  statGrad: { borderRadius:12 },
  statIcon: { width:40, height:40, borderRadius:12, justifyContent:'center',
    alignItems:'center', marginBottom:8 },
  statValue: { fontSize:20, fontWeight:'bold', color:'#111827', textAlign:'right' },
  statLabel: { fontSize:11, color:'#6b7280', textAlign:'right', marginTop:2 },
  ...
  tabBtn: { flex:1, flexDirection:'column', alignItems:'center', gap:4,
  ...
  filterBtn: { flexDirection:'row', alignItems:'center', gap:5, paddingHorizontal:12,
    paddingVertical:8, borderRadius:20, backgroundColor:'#f3f4f6', borderWidth:1, borderColor:'#e5e7eb' },
  ...
  itemQty: { fontSize:13, fontWeight:'bold', color:PRIMARY, backgroundColor: PRIMARY+'15',
  ...
  statusBtn: { flexDirection:'row', alignItems:'center', gap:4, paddingHorizontal:10,
    paddingVertical:7, borderRadius:10, backgroundColor:'#f9fafb',
  ...
  addBtnGrad: { flexDirection:'row', justifyContent:'center', alignItems:'center',
    height:52, gap:8 },
  addBtnText: { color:'#fff', fontWeight:'bold', fontSize:15 },

  productCard: { backgroundColor:'#fff', borderRadius:18, padding:14,
    flexDirection:'row-reverse', gap:12, marginBottom:10,
    shadowColor:'#000', shadowOpacity:0.05, shadowRadius:8, elevation:2 },
  productImg: { width:80, height:80, borderRadius:14 },
  productInfo: { flex:1 },
  productCatPill: { backgroundColor: PRIMARY+'15', borderRadius:8,
    paddingHorizontal:8, paddingVertical:3, alignSelf:'flex-end', marginBottom:4 },
  ...
  stockPill: { flexDirection:'row', alignItems:'center', gap:4,
    backgroundColor: PRIMARY+'12', borderRadius:8, paddingHorizontal:8, paddingVertical:3 },
  stockText: { fontSize:11, color:PRIMARY, fontWeight:'600' },
  productActions: { gap:8 },
  editBtn: { width:36, height:36, borderRadius:10,
    backgroundColor: PRIMARY+'15', justifyContent:'center', alignItems:'center' },
  deleteBtn: { width:36, height:36, borderRadius:10,
    backgroundColor:'#fef2f2', justifyContent:'center', alignItems:'center' },

  userCard: { borderRadius:20, marginBottom:12, overflow:'hidden',
    shadowColor:'#000', shadowOpacity:0.05, shadowRadius:8, elevation:2 },
  userGrad: { padding:16 },
  ...
  userAvatar: { width:52, height:52, borderRadius:26,
    backgroundColor: PRIMARY+'20', justifyContent:'center', alignItems:'center' },
  ...
  userStats: { flexDirection:'row', backgroundColor:'rgba(255,255,255,0.7)',
  ...
  confirmBtn: { borderRadius:16, marginTop:20, overflow:'hidden',
    shadowColor:PRIMARY, shadowOpacity:0.3, shadowRadius:10, elevation:5 },
