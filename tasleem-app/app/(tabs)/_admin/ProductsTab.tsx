    ...
    try {
      ...
      for (const asset of result.assets) {
        try {
          ...
          console.error('خطأ رفع صورة:', err);
        }
      }
            ...
            >
              ...
              <View style={s.productContent}>
                <View style={s.productHeader}>
                  ...
                  <View style={[
                    s.stockBadge, 
                    item.stock > 10 ? s.stockHigh : 
                    item.stock > 0 ? s.stockMed : 
                    s.stockLow
                  ]}>
                    <Text style={s.stockText}>{item.stock}</Text>
                  </View>
                </View>
                ...
                <View style={s.pricesGrid}>
                  <View style={s.priceItem}>
                    ...
                    <Text style={s.priceValue}>{(item.companyWholesalePrice || 0).toLocaleString()}</Text>
                  </View>

                  <View style={s.priceItem}>
                    <Ionicons name="storefront-outline" size={16} color="#6b7280" />
                    <Text style={s.priceLabel}>التاجر</Text>
                    <Text style={[s.priceValue, { color: PRIMARY }]}>{item.wholesalePrice.toLocaleString()}</Text>
                  </View>

                  <View style={s.priceItem}>
                    <Ionicons name="sparkles-outline" size={16} color="#6b7280" />
                    <Text style={s.priceLabel}>مقترح</Text>
                    <Text style={[s.priceValue, { color: ACCENT }]}>{(item.suggestedPrice || item.wholesalePrice).toLocaleString()}</Text>
                  </View>

                  <View style={s.priceItem}>
                    <Ionicons name="shield-checkmark-outline" size={16} color="#6b7280" />
                    <Text style={s.priceLabel}>أدنى</Text>
                    <Text style={[s.priceValue, { color: '#ef4444' }]}>{item.sellingPriceMin.toLocaleString()}</Text>
                  </View>
                </View>
                  ...
                  <TouchableOpacity 
                    ...
                    onPress={(e) => {
                      ...
                      Alert.alert('تأكيد الحذف', `هل تريد حذف "${item.name}"؟`, [
                        ...
                        { text: 'حذف', onPress: () => deleteProduct.mutate(item.id), style: 'destructive' }
                      ]);
                    }}
                  >
          ...
          <SafeAreaView style={{ flex: 1 }}>
            ...
            >
              ...
              <TextInput 
                ...
                onChangeText={(v: string) => setForm(p => ({ ...p, companyWholesalePrice: v }))}
              ...
              <TextInput 
                ...
                onChangeText={(v: string) => setForm(p => ({ ...p, wholesalePrice: v }))}
                keyboardType="number-pad" 
                textAlign="right" 
                placeholderTextColor="#9ca3af" 
              />
              ...
              <TextInput 
                ...
                onChangeText={(v: string) => setForm(p => ({ ...p, suggestedPrice: v }))}
                keyboardType="number-pad" 
                textAlign="right" 
                placeholderTextColor="#9ca3af" 
              />
              ...
              <TextInput 
                ...
                onChangeText={(v: string) => setForm(p => ({ ...p, sellingPriceMin: v }))}
                keyboardType="number-pad" 
                textAlign="right" 
                placeholderTextColor="#9ca3af" 
              />
              ...
              <TextInput 
                ...
                placeholder="0" 
                value={form.stock}
                onChangeText={(v: string) => setForm(p => ({ ...p, stock: v }))}
                keyboardType="number-pad" 
                textAlign="right" 
                placeholderTextColor="#9ca3af" 
              />
              ...
              <TextInput 
                ...
                placeholder="0" 
                value={form.discount}
                onChangeText={(v: string) => setForm(p => ({ ...p, discount: v }))}
                keyboardType="number-pad" 
                textAlign="right" 
                placeholderTextColor="#9ca3af" 
              />
              ...
              <TextInput 
                ...
                value={form.adLinks} 
                onChangeText={(v: string) => setForm(p => ({ ...p, adLinks: v }))}
                multiline 
                textAlign="right" 
                placeholderTextColor="#9ca3af" 
              />
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
                {form.images.map((uri: string, idx: number) => (
                  <View key={idx} style={s.imgPreview}>
                    ...
                    <TouchableOpacity 
                      style={s.imgRemove} 
                      onPress={() => setForm(p => ({ ...p, images: p.images.filter((_: string, i: number) => i !== idx) }))}
                    >
