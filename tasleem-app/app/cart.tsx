...
// ✅ مفتاح السلة مرتبط بالـ userId — كل حساب له سلة مستقلة
...
export default function CartScreen() {
  ...
  useEffect(() => { if (CART_KEY) loadCart(); }, [CART_KEY]);

  useFocusEffect(
    useCallback(() => { if (CART_KEY) loadCart(); }, [CART_KEY])
  );

  const updateQty = async (productId: number, newQty: number) => {
    if (!CART_KEY || newQty < 1) return;
    ...
    <SafeAreaView style={s.container} edges={['top', 'bottom']}>
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity
          ...
          <Ionicons name="trash-outline" size={20} color={cart.length ? "#ef4444" : "#d1d5db"} />
        </TouchableOpacity>
      </View>
...
const s = StyleSheet.create({
  ...
  trashBtn: { width: 40, height: 40, borderRadius: 12, backgroundColor: '#fef2f2', borderWidth: 1.5, borderColor: '#fecaca', justifyContent: 'center', alignItems: 'center' },
  trashBtnDisabled: { backgroundColor: '#f9fafb', borderColor: '#e5e7eb' },
  listContent: { padding: 16, paddingBottom: 140 },
  cartItem: { backgroundColor: '#fff', borderRadius: 16, padding: 12, marginBottom: 12, flexDirection: 'row', alignItems: 'center', gap: 12, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 8, elevation: 2, borderWidth: 1, borderColor: '#e8edf2', position: 'relative' },
  itemImage: { width: 70, height: 70, borderRadius: 12, backgroundColor: '#f3f4f6' },
  itemInfo: { flex: 1 },
  itemName: { fontSize: 14, fontWeight: '600', color: '#111827', marginBottom: 4, textAlign: 'right' },
  itemPrice: { fontSize: 13, color: PRIMARY, fontWeight: '600', marginBottom: 8, textAlign: 'right' },
  qtyBox: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 6 },
  qtyBtn: { width: 28, height: 28, borderRadius: 8, backgroundColor: '#f3f4f6', justifyContent: 'center', alignItems: 'center' },
  qtyText: { fontSize: 14, fontWeight: '600', color: '#111827', minWidth: 24, textAlign: 'center' },
  itemTotal: { fontSize: 12, color: '#6b7280', textAlign: 'right' },
  removeBtn: { position: 'absolute', top: 8, left: 8, padding: 4 },
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#fff', padding: 16, borderTopWidth: 1, borderTopColor: '#e8edf2', shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 12, elevation: 8 },
  ...
  emptyIconBox: { width: 90, height: 90, borderRadius: 45, backgroundColor: PRIMARY + '10', justifyContent: 'center', alignItems: 'center' },
  emptyTitle: { fontSize: 18, fontWeight: 'bold', color: '#374151' },
  emptyText: { fontSize: 13, color: '#9ca3af', textAlign: 'center' },
  shopBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: PRIMARY, borderRadius: 14, paddingHorizontal: 24, paddingVertical: 12, marginTop: 4 },
