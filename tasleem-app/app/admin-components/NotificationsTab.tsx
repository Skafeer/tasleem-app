      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.container}>
        ...
        <View style={s.logHeader}>
          <View style={s.logTitleRow}>
            ...
            {unread > 0 && <View style={s.redBadge}><Text style={s.redBadgeText}>{unread}</Text></View>}
          </View>
            ...
            {(['all', 'unread'] as const).map(f => (
              <TouchableOpacity key={f} style={[s.filterBtn, filter === f && s.filterBtnOn]} onPress={() => setFilter(f)}>
                <Text style={[s.filterText, filter === f && s.filterTextOn]}>
                  {f === 'all' ? `الكل (${all.length})` : `غير مقروء (${unread})`}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        {isLoading ? (
          ...
          shown.map((n: any) => {
            ...
            return (
              <View key={n.id} style={[s.notifCard, !n.is_read && s.notifCardUnread]}>
...
const s = StyleSheet.create({
  ...
  redBadge:     { backgroundColor: DANGER, borderRadius: 10, minWidth: 22, height: 22,
    justifyContent: 'center', alignItems: 'center', paddingHorizontal: 6 },
  redBadgeText: { fontSize: 11, color: '#fff', fontWeight: 'bold' },
  ...
  notifCardUnread: { borderRightWidth: 3, borderRightColor: PRIMARY, backgroundColor: '#f9fffe' },
