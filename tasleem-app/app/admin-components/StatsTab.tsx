...
function SectionTitle({ icon, title, color = PRIMARY }: { icon: any; title: string; color?: string }) {
  return (
    <View style={[s.sectionHeader, { borderRightColor: color }]}>
        ...
        {(['day', 'week', 'month', 'year'] as const).map((k, idx) => {
          ...
          return (
            <TouchableOpacity key={k} style={[s.tab, revenueTab === k && s.tabActive]} onPress={() => setRevenueTab(k)}>
              <Text style={[s.tabTxt, revenueTab === k && s.tabTxtActive]}>{labels[k]}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
const s = StyleSheet.create({
  ...
  monthLabel: { fontSize: 11, color: '#374151', fontWeight: '600', width: 32 },
  monthMeta: { width: 70, alignItems: 'flex-end', gap: 2 },
  ...
  barLabel: { fontSize: 11, color: '#374151', fontWeight: '600', width: 50 },
  barTrack: { flex: 1, height: 8, backgroundColor: '#f3f4f6', borderRadius: 4, overflow: 'hidden' },
  barFill: { height: 8, borderRadius: 4 },
  barMeta: { width: 60, alignItems: 'flex-end', gap: 1 },
