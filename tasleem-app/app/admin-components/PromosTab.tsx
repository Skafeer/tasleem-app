...
export default function PromosTab() {
  ...
  const handleSave = () => {
    if (!form.code.trim())           { toast.warning('يرجى إدخال الكود');        return; }
    if (!form.discountPercent.trim()) { toast.warning('يرجى إدخال نسبة الخصم'); return; }
    const pct = Number(form.discountPercent);
    if (isNaN(pct) || pct <= 0 || pct > 100) { toast.warning('نسبة الخصم يجب أن تكون بين 1 و 100'); return; }
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={s.modalOverlay}>
            <View style={s.modalCard}>
              ...
              <LinearGradient colors={[SECONDARY, '#e09000']} style={s.modalHeader} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
              ...
              <View style={s.modalFooter}>
                <TouchableOpacity style={s.saveBtn} onPress={handleSave} disabled={savePromo.isPending}>
                  {savePromo.isPending
...
const s = StyleSheet.create({
  ...
  addBtn:    { borderRadius: 14, overflow: 'hidden', margin: 12 },
  ...
  codeBox:      { flexDirection: 'row-reverse', alignItems: 'center', gap: 12 },
  ...
  editBtn:      { width: 34, height: 34, borderRadius: 10, backgroundColor: PRIMARY + '12', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: PRIMARY + '30' },
  deleteBtn:    { width: 34, height: 34, borderRadius: 10, backgroundColor: DANGER + '10', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: DANGER + '25' },

  divider: { height: 1, backgroundColor: '#f3f4f6', marginHorizontal: 14 },

  toggleRow:     { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 14, paddingVertical: 12 },
  toggleTrack:   { width: 46, height: 26, borderRadius: 13, backgroundColor: '#e5e7eb', justifyContent: 'center', padding: 2 },
  ...
  modalCard:    { backgroundColor: '#fff', borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: '85%' },
  modalHeader:  { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 16 },
  ...
  input:      { borderWidth: 1.5, borderColor: '#e5e7eb', borderRadius: 12, padding: 12, fontSize: 15, color: '#111827', backgroundColor: '#f9fafb' },
