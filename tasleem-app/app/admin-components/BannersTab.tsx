...
export default function BannersTab() {
  ...
  const openAdd  = () => { setEditBanner(null); setForm(EMPTY_FORM); setShowModal(true); };
  const openEdit = (b: any) => {
    setEditBanner(b);
    setForm({ title: b.title || '', imageUrl: b.imageUrl || '', link: b.link || '', isActive: Boolean(b.isActive), sortOrder: String(b.sortOrder ?? 0) });
    setShowModal(true);
  };
  ...
  if (isLoading) return <View style={s.center}><ActivityIndicator size="large" color={PRIMARY} /></View>;
              ...
              <ScrollView contentContainerStyle={s.sheetBody} showsVerticalScrollIndicator={false}>
                ...
                <TouchableOpacity style={s.uploadBox} onPress={pickImage} disabled={uploading}>
                  {uploading ? <ActivityIndicator color={PRIMARY} /> :
                   form.imageUrl ? <Image source={{ uri: form.imageUrl }} style={s.uploadPreview} resizeMode="cover" /> :
                   <>
                ...
                <TextInput style={s.input} value={form.link}
                  ...
                  textAlign="right" keyboardType="url" autoCapitalize="none" />
...
const s = StyleSheet.create({
  ...
  card:    { backgroundColor: '#fff', borderRadius: 18, marginBottom: 14, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.07, shadowRadius: 10, elevation: 3 },
  ...
  footer:        { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', padding: 12, gap: 8 },
  ...
  toggleBtn:  { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 10 },
  toggleTxt:  { fontSize: 11, fontWeight: '700' },
  editBtn:    { width: 32, height: 32, borderRadius: 10, backgroundColor: PRIMARY + '12', justifyContent: 'center', alignItems: 'center' },
  deleteBtn:  { width: 32, height: 32, borderRadius: 10, backgroundColor: DANGER + '10', justifyContent: 'center', alignItems: 'center' },

  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'flex-end' },
  sheet:   { backgroundColor: '#fff', borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: '92%' },

  sheetHeader: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
  sheetTitle:  { fontSize: 16, fontWeight: 'bold', color: '#111827' },
  closeBtn:    { width: 34, height: 34, borderRadius: 10, backgroundColor: '#f3f4f6', justifyContent: 'center', alignItems: 'center' },
  ...
  uploadBox:     { borderWidth: 2, borderColor: PRIMARY + '40', borderStyle: 'dashed', borderRadius: 16, height: 155, justifyContent: 'center', alignItems: 'center', backgroundColor: PRIMARY + '06', gap: 8, overflow: 'hidden' },
  ...
  activeToggle:  { flexDirection: 'row-reverse', alignItems: 'center', gap: 12, marginTop: 16, backgroundColor: '#f9fafb', borderRadius: 14, padding: 14, borderWidth: 1.5 },
